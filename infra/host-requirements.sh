...
install -d -m 755 /var/lib/host-agent/state
...
#    have auto-created an empty /var/lib/host-agent on first launch
#    of the renamed image; cp -a happily merges into it.
if [ -f /var/lib/dell-fans/state/base ] && [ ! -f /var/lib/host-agent/state/base ]; then
  echo "host-agent host-requirements: migrating /var/lib/dell-fans → /var/lib/host-agent (preserves EWMA)"
  cp -a /var/lib/dell-fans/. /var/lib/host-agent/
